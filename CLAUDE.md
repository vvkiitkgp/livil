...
| Two notification cards for one push (iOS) | `send-push` sends an ALERT payload (`aps.alert`, no `content-available`), which iOS renders itself. `remote-notification` is declared in `Info.plist` but inert. Add `content-available: 1` to that `aps` block and `setBackgroundMessageHandler` fires AND iOS renders the alert — two cards, one event. Send data-only OR alert, never both. |
